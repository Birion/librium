...
def get_genres(args) -> dict[str, list[GenreType] | int]:
    ...
    _genres,options["letters"], options["pagination"]  = get_raw(GenreService, args, filters)
    ...
    for genre in _genres:
        ...
        for b in genre.books:
            ...
            for s in b.series:
                book_info["series"].append({"name": s.series.name, "id": s.series.id, "idx": s.idx})
            for a in b.authors:
                book_info["authors"].append({"name": a.author.name, "id": a.author.id, "idx": a.idx})
            book_info["authors"].sort(key=lambda x: x["idx"])
            genre_books.append(book_info)

        sorted_list = sorted(genre_books, key=lambda x: x.get("series")[0]["name"] if x.get("series") else "")
        print(sorted_list)
        try:
            genre_books.sort(key=lambda x: (x["series"][0]["name"] if x.get("series") else "", x["series"][0]["idx"] if x.get("series") else 0, x["name"]))
        except IndexError:
